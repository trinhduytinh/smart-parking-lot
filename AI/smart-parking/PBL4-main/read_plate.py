...
if (len(LpImg)):
    ...
    roi = LpImg[0]                # biển số đã tách - màu gốc

    # tuong phan
    imgGrayscaleplate2, _ = Preprocess.preprocess(roi)
    imgThreshplate2 = cv2.adaptiveThreshold(imgGrayscaleplate2, 250, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                            cv2.THRESH_BINARY_INV, ADAPTIVE_THRESH_BLOCK_SIZE, ADAPTIVE_THRESH_WEIGHT)
    cv2.imshow("sau khi tang do tuong phan", imgThreshplate2)
    #
    ...
    thre_mor = cv2.morphologyEx(imgThreshplate2, cv2.MORPH_DILATE, kernel3)
    ...
    cred = credentials.Certificate("smart-parking-508e2-firebase-adminsdk-xzp00-d89e482ad6.json")

    firebase_admin.initialize_app(cred,{
	    'databaseURL':'https://smart-parking-508e2-default-rtdb.firebaseio.com'
    })

    handle = db.reference('/bienso')
    list_plate = handle.get()
    if plate_info in list_plate:
        print('Exist');
    else:
        print("Don't exist")
